...
export function mapScreen(state: Readonly<AppState>): string {
  ...
  return `<section class="screen" aria-labelledby="map-title"><div class="screen-header"><div><span class="eyebrow">Relative-signal aggregation</span><h1 id="map-title">${WORDING.estimateTitle}</h1><p>${WORDING.estimateBasis}</p></div></div>
    ...
    <div class="legend" style="margin-top:.8rem"><span style="color:var(--accent)">Confirmed direct / RSSI area</span><span style="color:#ffcf66">Bearing zone</span><span style="color:#40e0c2">Final-approach overlap</span><span style="color:var(--forwarded)">Forwarded</span><span style="color:var(--ambiguous)">Ambiguous</span><span>Unconfirmed</span></div>
