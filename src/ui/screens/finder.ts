...
export function finderScreen(state: Readonly<AppState>): string {
  ...
  return `<section class="screen" aria-labelledby="finder-title">
    ...
    ${state.activeSession?.mode === 'drive' ? `<article class="card ${state.smartWardrive.lastError ? 'warning' : ''} stack"><div class="row"><div><h2>Smart Wardrive</h2><p>${escapeHtml(state.smartWardrive.reason)}</p></div><span class="status-pill ${state.smartWardrive.active ? 'connected' : ''}">${state.smartWardrive.active ? 'foreground active' : 'paused'}</span></div><div class="cluster"><div class="metric"><span>Discovery runs</span><strong>${state.smartWardrive.discoveryRuns}</strong></div><div class="metric"><span>Observer polls</span><strong>${state.smartWardrive.observerPollRuns}</strong></div><div class="metric"><span>Fresh observers</span><strong>${state.remoteObserverAnalysis?.eligibleObserverCount ?? 0}</strong></div></div>${state.smartWardrive.lastError ? `<p class="warning-text">Last automation error: ${escapeHtml(state.smartWardrive.lastError)}</p>` : ''}<p class="fine-print">Automation pauses when this page is hidden, the radio disconnects, or the Drive session ends. Keep the phone unlocked and the app visible.</p></article>` : ''}
